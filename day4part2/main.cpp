...
int main(int argc, char **argv) {
	...
	LOGD("total_cards: %d", total_cards);
	...
	while (1) {
		...
		if (newline != NULL) {
			...
			int copies = copy_map[card_num];

			LOGD("card_num: %d, copies: %d", card_num, copies);
			...
			for (int i = card_num + 1; i < card_num + card.points + 1; i++) {
				if (i >= total_cards) {
					break;
				}

				LOGD("incrementing copies of %d", i);

				if (copy_map.find(i) == copy_map.end()) {
					copy_map[i] = 1;
				}

				// copy_map[i] = copy_map[i] + 1;
				copy_map[i] = copy_map[i] + copies;
			}
