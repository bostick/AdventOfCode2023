...
void analyzeRays(std::vector<Ray>& stable_rays, std::vector<Ray>& calculating_rays) {
...
int calculateEnergized(std::vector<Ray> stable_rays) {
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < contraption.row_count; i++) {
		...
		Ray ray = Ray{i, 0, RIGHT, -1, -1};

		stable_rays.clear();
		calculating_rays.clear();
		calculating_rays.push_back(ray);

		analyzeRays(stable_rays, calculating_rays);
		auto energized_count = calculateEnergized(stable_rays);
	...
	for (int i = 0; i < contraption.col_count; i++) {

		Ray ray = Ray{0, i, DOWN, -1, -1};
	...
	for (int i = 0; i < contraption.row_count; i++) {
		...
		Ray ray = Ray{contraption.col_count - 1, 0, LEFT, -1, -1};
	...
	for (int i = 0; i < contraption.col_count; i++) {
		...
		Ray ray = Ray{contraption.row_count - 1, i, UP, -1, -1};
